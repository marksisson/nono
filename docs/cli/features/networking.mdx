...
**What doesn't work:** Per-port filtering (`--open-port`, `--listen-port`) and proxy-based domain filtering (`--allow-domain`, `--network-profile`, `--credential`). Domain filtering is blocked by default on WSL2 — see [Credential Proxy on WSL2](/cli/internals/wsl2#credential-proxy-on-wsl2) for opt-in to degraded execution.
