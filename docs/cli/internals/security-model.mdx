...
In supervised mode, audit recording also lives in this trust boundary. The sandboxed child does not write its own audit log. Instead, the trusted parent records session metadata, supervisor-observed events, session-local integrity data, and the global audit ledger entry for that session.
...
- The audit log records command strings and supervisor-observed events. It does not yet hash the launched executable binary, shared libraries, interpreters, or dynamic runtime dependencies.
...
- attestation: not complete until an external signature or anchor is added
