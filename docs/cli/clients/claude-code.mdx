...
<Warning>
**macOS users:** Claude Code needs LaunchServices access when a session will need browser login. `nono` now auto-enables this for the built-in `claude-code` profile when it does not detect refresh-capable local auth. You can also pass the flag explicitly for first use or `claude /login`:
...
On macOS, `nono` auto-enables LaunchServices for the built-in `claude-code` profile when no refresh-capable local auth is detected. You can also enable it explicitly for the login session:
