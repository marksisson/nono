...
Disable the audit trail for this session. By default, every supervised execution records session metadata and audit events (command, timestamps, exit code, capability decisions, URL opens, network events) to `~/.nono/rollbacks/`. Use this flag to suppress audit recording entirely.
