...
```bash
# Default audit events + append-only integrity summary
nono run --audit-integrity --allow-cwd -- my-agent

# Strongest mode: integrity-protected audit log plus rollback snapshots
nono run --audit-integrity --rollback --profile claude-code -- claude
```

<Note>
  `--audit-integrity` protects the audit log that nono records. It does not replace `--rollback`, which is still the feature that captures filesystem state and restoreable snapshots.
</Note>

#### `--audit-sign-key`

Sign the session's audit Merkle root with a keyed signing key loaded by the supervisor. The resulting keyed DSSE attestation is written to `audit-attestation.bundle` in the audit session directory, and a summary is recorded in `session.json`.
...
<Note>
  `--audit-sign-key` signs the session audit Merkle root and session context. It does not, by itself, add an external timestamp, Rekor entry, or full runtime-closure attestation.
</Note>
