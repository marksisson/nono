...
For granular control, use `--network-profile` for host-level filtering, `--allow-net` to override proxy filtering for one session, or `--allow-port` for localhost IPC between sandboxes:
...
nono run --block-net --allow-port 3000 --allow . -- my-agent
```
